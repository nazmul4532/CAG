...
class Attacker:
    ...
    def _attack(self):
        ...
        # if torch.cuda.is_available():
            # self.attack.cuda_()
            # logger.info(
            #     f"Using CUDA GPU {torch.cuda.current_device()} my bruda."
            # )
        self.attack.cuda_()
        logger.info(
            f"Using CUDA GPU {torch.cuda.current_device()} my bruda."
...
# ##############################################################################################################
            try:
                if idx == 0:
                    ...
                    # signal.alarm(180)  # 180 seconds timeout
                    result = self.attack.attack(example, ground_truth_output)
                ...
                goal_result.goal_status = GoalFunctionResultStatus.SKIPPED
                result = SkippedAttackResult(goal_result)
# ##############################################################################################################
            if (
            ...
            # Always tick once per processed index (even if skipped/failed)
            pbar.update(1)
